...
- 2026-05-13 (evening) — OpenAlex Tier 2 experiment: NEGATIVE RESULT, not shipped. Built `openalex_fetch.py` (~250 LOC) mirroring `unpaywall_fetch_v2.py` structure; tested against getpaid's `needs_manual_pull.csv` residual (68 DOIs). Result: 0 net new downloads. 57 papers came back `oa_status=closed` (genuinely no OA), 11 returned publisher landing URLs that all 403'd on anonymous request — including one institutional repo (Hull). Conclusion: OpenAlex's broader green-OA repo coverage is real in theory but doesn't help for US sports-physiology corpora because the authors were under no deposit mandate. README now documents the institutional-access boundary honestly. **Don't re-invest in OpenAlex/CORE/Scholar wrappers** without first measuring the corpus-specific lift; this experiment is the cautionary baseline.
