...
class GenericSpacingWrapper(ToLaTeXConverter):
    ...
    def convert(self) -> str:
        # VENDORED FIX (2026-04-28): backport of asnunes/mathml-to-latex@5d1b794
        # ("fix: mo + mtable now render cases env instead of raw separators",
        # JS upstream v1.5.0, May 2025). Detects the linear-system pattern
        # `{ + mtable + empty mo` and renders as `\begin{cases}` instead of
        # the upstream's `\left\{...\right.` with raw alignment tabs (which
        # fails to compile under pdflatex). Until the Python port catches up
        # to the JS upstream, this fix lives here.
        if self._is_linear_system_pattern():
            return self._convert_as_linear_system()
        return " ".join(
            [
                self._adapter.to_latex_converter(child).convert()
                for child in self._math_element.children
    ...
    def _is_linear_system_pattern(self) -> bool:
        """`{ + mtable + empty closing mo` — the cases-env pattern."""
        children = self._math_element.children
        if len(children) != 3:
            return False
        first, second, third = children
        is_opening_brace = first.name == "mo" and first.value.strip() == "{"
        is_mtable = second.name == "mtable"
        is_empty_closing = third.name == "mo" and third.value.strip() == ""
        return is_opening_brace and is_mtable and is_empty_closing

    def _convert_as_linear_system(self) -> str:
        mtable_child = self._math_element.children[1]
        rows = " \\\\\n".join(
            self._adapter.to_latex_converter(child).convert()
            for child in mtable_child.children
        )
        return "\\begin{cases}\n" + rows + "\n\\end{cases}"
